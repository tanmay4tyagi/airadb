...
(function(window){
  ...
  var QRUtil={
    ...
    getLengthInBits:function(mode,type){return(type>=1&&type<=9)?8:(type>=10&&type<=26)?16:16;},
  ...
  QRRSBlock.RS_BLOCK_TABLE=[[1,26,19],[1,44,34],[1,70,55],[1,100,80],[1,134,108],[2,86,68],[2,98,78],[2,121,97],[2,146,116],[2,86,68,2,87,69]];
  QRRSBlock.getRSBlocks=function(typeNumber,errorCorrectLevel){
    var rsBlock=QRRSBlock.RS_BLOCK_TABLE[typeNumber-1];
    if(!rsBlock)rsBlock=QRRSBlock.RS_BLOCK_TABLE[3];
  ...
  window.QRCode = function(target, options){
    ...
    function render(text){
      var qr = new QRCodeModel(4, opt.correctLevel);
      qr.addData(text);
      qr.make();
      ...
      var tileW = opt.width / count;
      var tileH = opt.height / count;
      ...
      for(var r=0; r<count; r++){
        for(var c=0; c<count; c++){
          ctx.fillStyle = qr.isDark(r,c)?opt.colorDark:opt.colorLight;
    ...
    if(opt.text) render(opt.text);
    return { makeCode: render };
  };
})(window);
